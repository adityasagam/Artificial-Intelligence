...
def main(argv):
    ...
    additional_knowledge = logical_expression()
    additional_knowledge.connective = ['and']
    for line in input_file:
        # Skip comments and blank lines. Consider all line ending types.
        if line[0] == '#' or line == '\r\n' or line == '\n' or line == '\r':
            continue
        ...
        subexpression = read_expression(line.rstrip('\r\n'), counter)
        additional_knowledge.subexpressions.append(subexpression)
        #knowledge_base.subexpressions.append(subexpression)
    input_file.close()
    
    # Verify it is a valid logical expression
    if not valid_expression(additional_knowledge):
        sys.exit('invalid additional knowledge')
    '''
    knowledge_base_dict = init_truth_values(knowledge_base, None)

    additional_knowledge_dict = init_truth_values(additional_knowledge, True)
    print '\n\n',additional_knowledge_dict
    
    knowledge_base.subexpressions.extend(additional_knowledge.subexpressions)
    knowledge_base_dict.update(additional_knowledge_dict)
    ...
    check_true_false(knowledge_base, statement, additional_knowledge)
    '''
    print(statement.symbol)
    print(statement.connective)
    print(statement.subexpressions[0].symbol)
    print(statement.subexpressions[0].connective)
    print(statement.subexpressions[0].subexpressions)
    print '\n\n'
    print(statement.subexpressions[1].symbol)
    print(statement.subexpressions[1].connective)
    print(statement.subexpressions[1].subexpressions[0].symbol)'''
